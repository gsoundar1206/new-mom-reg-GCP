...
class ResponseCookieAdapter(object):
    ...
    def getheaders(self, header):
        print("Inside get hedders methord",header)
...
class TestApp(webtest.TestApp):
    ...
    def _do_httplib_request(self, req):
        ...
        #headers1 = dict((name, val) for name, val in req.headers.iteritems())
        #print("values +++++++++++",headers1)
        ...
        #return res
        ###############
        #print("object values ",webresp.getheaders())
        #print("object keys ",dict(webresp.getheaders()).keys)
        for headername in dict(webresp.getheaders()).keys():
            #print("header name =",headername)
            for headervalue in dict(webresp.getheaders()).get(headername):
            #for headervalue in webresp.msg.getheaders(headername):
                #print("headervalue   name =",headervalue)
                response_headers.append((headername, headervalue))
        res.headerlist = response_headers
        res.errors = ''
        return res
