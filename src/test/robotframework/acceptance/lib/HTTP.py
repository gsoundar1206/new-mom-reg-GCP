...
import urllib3 as urlparse
#from urllib import pasrse as urlparse
...
class HTTP:
    ...
    *JSON*
    ...
    class Context(object):
        ...
        def post_process_request(self, response):
            ...
            if next_request_should == True:
                assert int(self.response.status[0:3]) < 1000, \
    ...
    def response_body_should_contain(self, should_contain):
        ...
        print("The response body ==",self.response.body)
        print("The should contain ==",should_contain)
        logger.debug('Testing whether "%s" contains "%s".' % (
            self.response.body, should_contain))

        assert should_contain in str(self.response.body), \
